...
public class GeneralLedgerService(IGeneralLedgerRepository generalLedgerRepository) : IGeneralLedgerService
{
    ...
    /// If the account ID is already in use, throws an InvalidOperationException with an appropriate error message. 
    ...
    public async Task AddGeneralLedgerAccountAsync(GeneralLedgerAccount account)
    {
        ...
        if (isInUse)
            throw new InvalidOperationException($"General ledger account with ID {account.Id} is already in use and cannot be added.");
